...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Delivery Rate</h1>
                <p class="mt-1 text-sm text-gray-600">Update shipping rate details</p>
            </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.delivery.rates.update', $rate) }}" method="POST">
        @csrf
        @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="delivery_zone_id" 
                            ...
                            @foreach($zones as $zone)
                                <option value="{{ $zone->id }}" {{ old('delivery_zone_id', $rate->delivery_zone_id) == $zone->id ? 'selected' : '' }}>
                                    {{ $zone->name }} ({{ $zone->code }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="delivery_method_id" 
                            ...
                            @foreach($methods as $method)
                                <option value="{{ $method->id }}" {{ old('delivery_method_id', $rate->delivery_method_id) == $method->id ? 'selected' : '' }}>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="handling_fee" 
                               id="handling_fee" 
                               value="{{ old('handling_fee', $rate->handling_fee) }}"
                               step="0.01"
                               min="0"
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="insurance_fee" 
                               id="insurance_fee" 
                               value="{{ old('insurance_fee', $rate->insurance_fee) }}"
                               step="0.01"
                               min="0"
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="cod_fee" 
                               id="cod_fee" 
                               value="{{ old('cod_fee', $rate->cod_fee) }}"
                               step="0.01"
                               min="0"
