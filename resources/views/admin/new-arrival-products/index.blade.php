...
@section('title', 'New Arrival Products Management')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">New Arrival Products Management</h1>
            <p class="text-gray-600 mt-1">Manage products displayed in "New Arrivals" section on homepage</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-lg shadow-md p-6">
                <h2 class="text-xl font-semibold text-gray-900 mb-4">Add Product to New Arrivals</h2>
                
                <!-- Livewire Product Selector -->
                @livewire('admin.new-arrival-product-selector')
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-lg shadow-md p-6">
                <h2 class="text-xl font-semibold text-gray-900 mb-4">
                    Current New Arrivals ({{ $newArrivalProducts->count() }})
                </h2>
                ...
                @if($newArrivalProducts->isEmpty())
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-600">No new arrival products added yet. Add your first product above.</p>
                    </div>
                @else
                    <div id="newarrival-list" class="space-y-3">
                        @foreach($newArrivalProducts as $newArrival)
                            <div 
                            ...
                            >
                                ...
                                <div class="flex items-center space-x-2">
                                    <!-- Toggle Status -->
                                    <button 
                                        onclick="toggleStatus({{ $newArrival->id }})"
                                        class="px-3 py-1 rounded-md text-sm font-medium transition {{ $newArrival->is_active ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                                    >
                                        {{ $newArrival->is_active ? 'Active' : 'Inactive' }}
                                    </button>

                                    <!-- Delete -->
                                    <form action="{{ route('admin.new-arrival-products.destroy', $newArrival) }}" method="POST" class="inline" id="delete-form-{{ $newArrival->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button 
                                            type="button"
...
<script>
    ...
    if (list) {
        new Sortable(list, {
            ...
            onEnd: function(evt) {
                ...
                fetch('{{ route("admin.new-arrival-products.update-order") }}', {
                    method: 'POST',
                    headers: {
    ...
    function toggleStatus(id) {
        fetch(`/admin/new-arrival-products/${id}/toggle-status`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
