...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 mt-2">Edit Attribute: {{ $attribute->name }}</h1>
        <p class="text-sm text-gray-600 mt-1">Update attribute information and values</p>
    </div>

    <form action="{{ route('admin.attributes.update', $attribute) }}" method="POST" x-data="attributeForm()">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" 
                        ...
                        <option value="button" {{ $attribute->type === 'button' ? 'selected' : '' }}>Button/Badge</option>
                    </select>
                    @error('type')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="space-y-3">
                <template x-for="(value, index) in values" :key="index">
                    <div class="flex items-center gap-3">
                        <input type="hidden" :name="'values[' + index + '][id]'" x-model="value.id">
        ...
        <div class="flex items-center justify-end gap-4 mt-6">
            ...
            <button type="submit" 
                ...
                <i class="fas fa-save mr-2"></i> Update Attribute
            </button>
        </div>
    </form>
</div>
...
function attributeForm() {
    return {
        type: '{{ $attribute->type }}',
        values: @json($attribute->values->map(function($v) {
            return [
                'id' => $v->id,
                'value' => $v->value,
        ...
        addValue() {
            this.values.push({ id: null, value: '', color_code: '#000000' });
        },
        removeValue(index) {
