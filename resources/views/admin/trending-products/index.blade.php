...
@section('title', 'Trending Products Management')
...
<div class="container mx-auto px-4 py-8">
    ...
    <x-admin.section-settings
        ...
        toggleRoute="{{ route('admin.trending-products.toggle-section') }}"
        updateTitleRoute="{{ route('admin.trending-products.update-title') }}"
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Trending Products Management</h1>
            <p class="text-gray-600 mt-1">Manage products displayed in "Trending now" section on homepage</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <div class="bg-white rounded-lg shadow-md p-6">
                <h2 class="text-xl font-semibold text-gray-900 mb-4">Add Product to Trending</h2>
                
                <!-- Livewire Product Selector -->
                @livewire('admin.trending-product-selector')
        ...
        <div class="lg:col-span-2">
            <div class="bg-white rounded-lg shadow-md p-6">
                <h2 class="text-xl font-semibold text-gray-900 mb-4">
                    Current Trending Products ({{ $trendingProducts->count() }})
                </h2>
                ...
                @if($trendingProducts->isEmpty())
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-600">No trending products added yet. Add your first product above.</p>
                    </div>
                @else
                            ...
                            >
                                <div class="flex items-center space-x-4 flex-1">
                                    ...
                                    @if($trending->product)
                                        ...
                                        <!-- Product Info -->
                                        <div class="flex-1">
                                            <h3 class="font-semibold text-gray-900">{{ $trending->product->name }}</h3>
                                            <div class="flex items-center space-x-3 mt-1">
                                                ...
                                                @if($trending->product->variants->first())
                                                    <span class="text-xs text-gray-500">
                                                        Stock: {{ $trending->product->variants->first()->stock_quantity }}
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
                                ...
                                <div class="flex items-center space-x-2">
                                    <!-- Toggle Status -->
                                    <button 
                                        onclick="toggleStatus({{ $trending->id }})"
                                        class="px-3 py-1 rounded-md text-sm font-medium transition {{ $trending->is_active ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}"
                                    >
                                        {{ $trending->is_active ? 'Active' : 'Inactive' }}
                                    </button>

                                    <!-- Delete -->
                                    <form action="{{ route('admin.trending-products.destroy', $trending) }}" method="POST" class="inline" id="delete-form-{{ $trending->id }}">
                                        @csrf
                                        @method('DELETE')
                                        <button 
                                            type="button"
                                            onclick="confirmDelete({{ $trending->id }}, '{{ $trending->product ? addslashes($trending->product->name) : 'Deleted Product' }}')"
...
<script>
    ...
    if (list) {
        new Sortable(list, {
            ...
            onEnd: function(evt) {
                ...
                fetch('{{ route("admin.trending-products.update-order") }}', {
                    method: 'POST',
                    headers: {
    ...
    function toggleStatus(id) {
        fetch(`/admin/trending-products/${id}/toggle-status`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
