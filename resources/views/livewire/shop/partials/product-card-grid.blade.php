    ...
    <div class="relative aspect-square bg-gray-100">
        ...
        <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition">
            <button onclick="this.querySelector('svg').classList.toggle('fill-red-500'); this.querySelector('svg').classList.toggle('text-red-500'); Livewire.dispatch('toggle-wishlist', { productId: {{ $product->id }}, variantId: {{ $variant->id ?? 'null' }} })"
                    class="p-2 bg-white rounded-full shadow-md hover:scale-110 transition-all duration-200"
    ...
    <div class="p-4">
        ...
        @if($variant && $canAddToCart)
            ...
            <button wire:click="addToCart({{ $product->id }}, {{ $variant->id }}, 1)"
                    ...
                    class="w-full px-4 py-2 {{ $isInCart ? 'bg-blue-600 hover:bg-blue-700' : 'bg-green-600 hover:bg-green-700' }} text-white text-sm font-medium rounded-lg transition disabled:opacity-50 flex items-center justify-center">
                @if($isInCart)
