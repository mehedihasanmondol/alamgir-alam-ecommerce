...
class DeliverySelector extends Component
{
    ...
    protected $listeners = ['cartUpdated' => 'refreshDelivery'];
    ...
    public function mount($cartTotal = 0, $cartWeight = 0, $itemCount = 0)
    {
        ...
        // Load user's default delivery preferences if logged in
        if (Auth::check() && Auth::user()->default_delivery_zone_id) {
    ...
    public function saveToSession()
    {
        ...
        // Notify inline component to update
    ...
    {
        if (!Auth::check()) {
            $this->dispatch('show-toast', type: 'error', message: 'Please login to save delivery preferences');
            return;
        }

        if (!$this->selectedZoneId || !$this->selectedMethodId) {
            $this->dispatch('show-toast', type: 'error', message: 'Please select delivery zone and method');
            return;
        }

        Auth::user()->update([
            'default_delivery_zone_id' => $this->selectedZoneId,
            'default_delivery_method_id' => $this->selectedMethodId,
        ]);

        $this->dispatch('show-toast', type: 'success', message: 'Delivery preferences saved successfully!');
    }

    public function refreshDelivery($cartTotal, $cartWeight, $itemCount)
    {
        $this->cartTotal = $cartTotal;
        $this->cartWeight = $cartWeight;
        $this->itemCount = $itemCount;
    ...
    public function render()
    {
        ...
        return view('livewire.cart.delivery-selector');
    }
}
